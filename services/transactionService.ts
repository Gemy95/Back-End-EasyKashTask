...
public static getUserTransactions = async (queryParameters)=>{
  let per_page = Number(queryParameters.per_page) < 1 && (Number(queryParameters.per_page)) != NaN ? 10 : queryParameters.per_page;;  
  let current_page = Number(queryParameters.page) < 1 && (Number(queryParameters.page)) != NaN  ? 1 : queryParameters.page;
